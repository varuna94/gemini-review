...
- **Depends on:** ~~1.4.0 결과 계약 · diff 해시~~ — **선결 조건이 모두 충족됐다(1.5.0).**
  `_meta.diff_sha256` 이 리뷰에 쓴 같은 바이트로 계산된다.
  ⚠ 해시 대상은 **스크립트가 디코딩·재인코딩한 UTF-8** 이다. hook 을 `git diff | sha256sum`
  으로 짜면 값이 맞지 않는다 — `_collect_diff` 와 `_diff_bytes` 를 같이 써야 한다.
