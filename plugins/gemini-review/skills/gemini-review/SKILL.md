...
| 코드 | 뜻 |
|---|---|
| 0 | 통과 — **주 모델의** `approve` 또는 `approve_with_comments` |
...
| 8 | `--staged` 인데 스테이징이 비었다 — 리뷰가 안 된 것이다 (`--allow-empty` 면 0) |
...
플러그인 hook 이 검사한다. 스테이징된 변경과 **같은 내용으로 통과한(exit 0) 리뷰**가 없으면
`⛔ 커밋 게이트(gemini-review)가 이 명령을 막았다.` 와 `원인:` · `해결:` 줄을 남기고 막는다.

- 막히면 `해결:` 줄대로 한다. 보통은 `--staged` 리뷰를 통과시킨 뒤 **스테이징을 바꾸지 않고**
  `git commit` 을 **단독 명령**으로 다시 실행하는 것이다. `git add … && git commit` 처럼 한 명령에
  섞으면 게이트가 확인할 수 없어 막는다.
- ⛔ **게이트를 끄거나 우회하지 말 것.** 설정 변경 · 결과 파일 편집 · 다른 경로로 커밋하기는 모두
  사용자의 결정이다. 리뷰를 돌릴 수 없는 저장소라면 그 사실을 사용자에게 보고한다.
- 리뷰가 exit 4 로 끝나 통과를 받지 못하면 「판정 결과 읽는 법」대로 재시도하고, 그래도 안 되면
  "리뷰가 수행되지 않아 커밋하지 못했다" 고 보고한다.
